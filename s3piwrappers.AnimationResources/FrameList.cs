...
namespace s3piwrappers
{
    ...
    {
        private readonly CurveType mType;

        public FrameList(EventHandler handler, CurveType type) : base(handler) { mType = type; }

        public FrameList(EventHandler handler, CurveType type, IEnumerable<Frame> ilt) : base(handler, ilt) { mType = type; }

        public FrameList(EventHandler handler, CurveType type, Stream s, CurveDataInfo info, IList<float> floats) : base(handler)
        {
            mType = type;
            Parse(s, info, floats);
        }
        ...
        public override void Add() { Add(new object[] {Frame.CreateInstance(0, this.handler, mType)}); }

        protected virtual Frame CreateElement(Stream s, CurveDataInfo info, IList<float> floats) { return Frame.CreateInstance(0, handler, mType, s, info, floats); }

...
namespace s3piwrappers.SceneGraph.Nodes
{
    public class CatalogBrushNode : CatalogNode
    {
        ...
        public static readonly uint[] brushTIDs = new uint[]
        {
        ...
        private static Dictionary<ulong, SpecificResource> CTPTBrushIndexToPair0
            = new Dictionary<ulong, SpecificResource>();
        // Brushes shown in catalog
        private static Dictionary<ulong, SpecificResource> CTPTBrushIndexToPair1
            = new Dictionary<ulong, SpecificResource>();
            ...
            for (i = 0; i < ppts.Count; i++)
            {
                ...
                for (j = 0; j < lrs.Count; j++)
                {
                    ...
                    if (sr.Resource != null)
                    {
                        ...
                        ulong brushIndex = (ulong)sr.Resource["CommonBlock.NameGUID"].Value;
        ...
        {
            private SpecificResource brushPairCounterPart;
        ...
        public override bool CommitChanges()
        {
            ...
            {
                string uniqueObject = (string)base.resource["CommonBlock.Name"].Value;

                byte status = (byte)base.resource["CommonBlock.BuildBuyProductStatusFlags"].Value;
                uint brushIndex = System.Security.Cryptography.FNV32.GetHash(uniqueObject) << 1;
                if ((status & BBProdStatusShowInCatalog) != 0)

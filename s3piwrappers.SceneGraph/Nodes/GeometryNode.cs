...
namespace s3piwrappers.SceneGraph.Nodes
{
    public class GeometryNode : DefaultNode
    {
        ...
        private IResourceConnection GEOM_getNormalMap()
        {
            ...
            {
                Diagnostics.Show("GEOM resource not found");
                return null;
            }
            if (!(base.resource is GeometryResource))
            {
                Diagnostics.Show("GEOM resource is not a GeometryResource");
                return null;
            }
            var geomResource = base.resource as GeometryResource;
            if (geomResource.ChunkEntries.Count != 1)
            {
                Diagnostics.Show("GeometryResource should have 1 Chunk, found " + geomResource.ChunkEntries.Count);
                return null;
            }
            var geom = geomResource.ChunkEntries[0].RCOLBlock as GEOM;
            if (geom == null)
            {
                Diagnostics.Show("GEOM RCOL block not found");
                return null;
            }
            ...
            if (mtnf == null)
            {
                Diagnostics.Show("MTNF expected but was null");
                return null;
            }
            ShaderData sd = mtnf.SData.Find(IsNormalMap);
            if (sd == null)
            {
                Diagnostics.Show("NormalMap not found");
                return null;
            }
            var tr = sd as ElementTextureRef;
            if (tr == null)
            {
                Diagnostics.Show("NormalMap not an ElementTextureRef");
                return null;
            }
            ...
            if (index < 0)
            {
                Diagnostics.Show(String.Format("NormalMap index read {0}, expected >= 0", index));
                return null;
            }
            if (index >= geom.TGIBlocks.Count)
            {
                Diagnostics.Show(String.Format("NormalMap index read {0}, expected < {1}", index, geom.TGIBlocks.Count));
                return null;
            }
        ...
        private void SlurpKinRCOLTGIBlocks()
        {
            ...
            if (rcol != null)
            {
                ...
                {
                    if (GetChunkType(rcol.ChunkEntries[i].TGIBlock) == ChunkEntryType.Kindred)
                    {
